...
interface ElectronAPI {
  ...
  onTriggerChatSolve: (callback: (data: { path: string; preview: string }) => void) => () => void
  ...
  onStealthInput: (callback: (event: any, data: { type: string; value: string }) => void) => () => void
...
contextBridge.exposeInMainWorld("electronAPI", {
  ...
  onTriggerChatSolve: (
    ...
    const subscription = (_: any, data: { path: string; preview: string }) =>
      callback(data)
    ipcRenderer.on("trigger-chat-solve", subscription)
    return () => {
      ipcRenderer.removeListener("trigger-chat-solve", subscription)
    }
  },
  ...
  onStealthInput: (callback: (event: any, data: { type: string; value: string }) => void) => {
    const subscription = (_: any, data: { type: string; value: string }) => callback(_, data)
    ipcRenderer.on("stealth-input", subscription)
    return () => {
      ipcRenderer.removeListener("stealth-input", subscription)
    }
  },
