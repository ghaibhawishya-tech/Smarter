...
export function initializeIpcHandlers(appState: AppState): void {
  ...
  ipcMain.handle("set-content-protection", async (_, enabled: boolean) => {
    appState.setContentProtection(enabled)
  })

  ipcMain.handle("drop-focus", async () => {
    appState.dropFocus()
  })

  ipcMain.handle("start-stealth-mode", async () => {
    appState.inputInterceptor.startIntercepting()
  })

  ipcMain.handle("stop-stealth-mode", async () => {
    appState.inputInterceptor.stopIntercepting()
  })

  ipcMain.handle("set-focusable", async (_, focusable: boolean) => {
    appState.setFocusable(focusable)
  })
